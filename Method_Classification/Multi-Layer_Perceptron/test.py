...
# Perform prediction using the model
prediction = model.predict(new_features_scaled)
probability = prediction[0][0]  # Probability of the positive class (tidak_cadel)

# Output prediction result and percentage
if probability >= 0.5:
    print(f"Audio tidak cadel (Probability: {probability * 100:.2f}%)")
else:
    print(f"Audio cadel (Probability: {(1 - probability) * 100:.2f}%)")

# Visualize the MFCC features as average amplitude
